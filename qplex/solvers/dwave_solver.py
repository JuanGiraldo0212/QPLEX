...
from dwave.system import LeapHybridCQMSampler, LeapHybridBQMSampler, LeapHybridDQMSampler
from dimod import ConstrainedQuadraticModel, QuadraticModel, DiscreteQuadraticModel, BinaryQuadraticModel
...
class DWaveSolver(Solver):
    ...
    def parse_input(self, model) -> any:
        ...
        if len(list(model.iter_constraints())) > 0:
            ...
            for constraint in model.iter_constraints():
                ...
                parsed_model.add_constraint(const_qm, sense=sense, rhs=rhs, label=constraint.lpt_name)
        else:
            model_type = VAR_TYPE['B']
            if any(VAR_TYPE[var.vartype.cplex_typecode] == VAR_TYPE['I'] for var in model.iter_variables()):
                model_type = VAR_TYPE['I']
            parsed_model = DiscreteQuadraticModel() if model_type == VAR_TYPE['I'] else BinaryQuadraticModel(
                vartype='BINARY')
    ...
    def parse_objective(self, model, parsed_model):
        if type(parsed_model) is BinaryQuadraticModel:
            ...
            for var in model.iter_variables():
                parsed_model.add_variable(VAR_TYPE[var.vartype.cplex_typecode], var.name, lower_bound=var.lb,
                                          upper_bound=var.ub)

        linear_terms = model.get_objective_expr().iter_terms()
        sense_multiplier = 1 if model.objective_sense.name == "Minimize" else -1
        ...
        for term in model.get_objective_expr().iter_quad_triplets():
            parsed_model.set_quadratic(term[0].name, term[1].name, term[2] * sense_multiplier)
